...
class TicTacToePolicy(dc.rl.Policy):
  def create_layers(self, state, **kwargs):
    ...
    d2 = Dense(in_layers=[d1], activation_fn=tf.nn.relu,
               normalizer_fn=tf.nn.l2_normalize, normalizer_params={"dim": 1},
               out_channels=64)
    d3 = Dense(in_layers=[d2], activation_fn=tf.nn.relu,
               normalizer_fn=tf.nn.l2_normalize, normalizer_params={"dim": 1},
               out_channels=32)
    d4 = Dense(in_layers=[d3], activation_fn=tf.nn.relu,
               normalizer_fn=tf.nn.l2_normalize, normalizer_params={"dim": 1},
               out_channels=16)
...
def main():
  ...
  while end - start < timeout:
    a3c = dc.rl.A3C(env,
                    policy,
                    discount_factor=0,
                    model_dir="/home/leswing/tictactoe")
