...
from deepchem.feat.sanifix import sanitize
...
def load_molecule(molecule_file, add_hydrogens=True,
    ...
    def add_hydrogens_func(m):
        if add_hydrogens:
            return Chem.AddHs(m)
        return m

    if ".mol2" in molecule_file:
        m = Chem.MolFromMol2File(str(molecule_file), False)
        m = sanitize(m)
        m = add_hydrogens_func(m)
        AllChem.EmbedMolecule(m, AllChem.ETKDG())
    elif ".sdf" in molecule_file:
        m = Chem.SDMolSupplier(str(molecule_file), False)[0]
        m = sanitize(m)
        m = add_hydrogens_func(m)
        AllChem.EmbedMolecule(m, AllChem.ETKDG())
    elif ".pdbqt" in molecule_file:
        raise ValueError("No Longer Support .pdbqt files %s" % molecule_file)
    elif ".pdb" in molecule_file:
        m = Chem.MolFromPDBFile(str(molecule_file), False)
    else:
        raise ValueError("Unrecognized file type")

    # TODO(LESWING) figure out how important/how to add charges
    # if calc_charges:
...
def _reflect_molecule(mol, mol_coordinates_list):
